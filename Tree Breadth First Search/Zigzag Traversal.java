...
class Solution {
    public List<List<Integer>> zigzagLevelOrder(TreeNode root) {
        ...
        while(treeNodesQueue.size()!=0)
        {
            ...
            for(int i=0;i<levelLength;i++)                      // This is done so that we can seperate the nodes in levels and it wont go on 
                                                                //infiniteley becos the level length is calculated only once
            {                                                   // Also the level length is bascially the number of parent nodes
                                                                // We did this inner for loop becos here we need level by level data but 
                                                                //usually in bfs we dont have inner for loop
